...
var Converter = exports.Converter = function (image, options) {
    ...
    if(this.options.processor === 'sharp') {
      this.sharp = sharp;
      this.sharp.prototype.write = this.sharp.prototype.toFile;
      this.sharp.prototype.autoOrient = function() { return this.rotate(); };
      this.sharp.prototype.size = this.sharp.prototype.metadata;
      ['coalesce', 'filter', 'repage', 'interlace'].forEach(function(methodName) {
          this.sharp.prototype[methodName] = function() {
            if(this.options.debug) {
              console.error('ERROR, method', methodName, ' not implemented in Sharp.', 'arguments:', arguments);
            }
            return this;
          };
      }.bind(this));
...
Converter.prototype.save = function (output, callback) {
    var dir = path.dirname(output)
      , image = this.options.processor === 'sharp' ? this.sharp(this.image) : this.gm(this.image)
    ...
    mkdirp(dir, function () {
        ...
        image.size(function (err, size) {
            ...
            if (operation.crop_width || operation.crop_height) {
                ...
                this.options.processor === 'sharp' ? image.resize(operation.crop_width, operation.crop_height, { fit: 'cover', position: sharp.gravity.center }) : image.crop(operation.crop_width, operation.crop_height, operation.x, operation.y);
            ...
            //Only needed for Graphicsmagick
            if (this.options.processor === 'gm' && !operation.width && operation.height) {
                operation.width = Math.ceil(operation.height / size.height * size.width);
            }
            if (this.options.processor === 'gm' && !operation.height && operation.width) {
            ...
            // By default Sharp strips metadata, needed to get orienttion correct
            if(image.withMetadata) {
              image.withMetadata();
            }
